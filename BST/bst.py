...
class BST:
    ...
    def __init__(self, list_of_objects = None, bst = None):
        ...
        1. empty: CharCount()
        2. from a list: CharCount(list_of_objects = a_list)
        3. from a BST tree: CharCount(bst = a_bst)
        """
    ...
    def search(self, key):
        ...
        Start the recursive search for the key at the root node
        """

        return self.search_rec(self.root, key)


    def search_rec(self, current, key):
        """
        Recursively finds the key in the BST,
        ...
        if current is not None:
            ...
            comp = key.compare(current.element)

            if comp < 0:
                found = self.search_rec(current.left, key)
            elif comp > 0:
                found = self.search_rec(current.right, key)
            else:
                found = True
